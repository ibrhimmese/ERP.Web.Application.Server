...
internal sealed class GetAllProductQueryHandler(
    ...
    ) : IRequestHandler<GetAllProductQuery, Result<List<GetAllProductQueryResponse>>>
{
    ...
    {
        List<Product> products = await productRepository.GetAll().OrderBy(p=>p.ProductType).ToListAsync(cancellationToken);
        ...
        {
            decimal stock = await stockMovementRepository.Where(p => p.ProductId == item.Id)
